...
while running:
    ...
    for player in players:
        print("\n")
        print(bcolors.HEADER + "Player Name: " + player.name + bcolors.ENDC)
        player.choose_action()
        choice = input("Choose action: ")
        while len(choice) == 0 or int(choice) > 3:
            choice = input("Your input in invalid, please choose action 1, 2 or 3: ")
        # elif int(choice) < 1:
        #    continue
        index = int(choice) - 1
        ...
        if index == 0:
            ...
            player.choose_magic()
            choose_magic = input("Choose magic: ")
            while len(choose_magic) == 0:
                choose_magic = input("Your input in invalid, please choose valid magic: ")
            while int(choose_magic) > len(player_spells) - 1:
                choose_magic = input("Your input in invalid, please choose valid magic: ")
            # elif int(choose_magic) - 1 < 0:
                # continue
            # magic_dmg = player.generate_spell_damage(magic_choice)
            # spell = player.get_spell_name(magic_choice)
            # cost = player.get_spell_mpcost(magic_choice)
            magic_choice = int(choose_magic) - 1
            ...
            if spell.type == "white":
                ...
                print("You attacked " + enemies[enemy].name + " for ", magic_dmg, "points of damage.")
    ...
    for enemy in enemies:
        ...
        if enemy_choice == 0:
            ...
            print(enemy.name + " attacks for", enemy_dmg, "points at", players[target].name)
            ...
            if spell.type == "white":
                ...
                if players[target].get_hp() == 0:
                    ...
                    del players[target]
            #print("Enemy chose", spell, "to deal", magic_dmg)
